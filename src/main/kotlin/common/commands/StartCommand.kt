...
object StartCommand : MessageCommand(State.Setup.Setup) {
    private const val approveChar = '✔'
    private const val rejectChar = '❌'
        ...
        with(message) {
            ...
            if (args.lastOrNull() == "now") {
                if (message.author != steadfast) {
                    ...
                    channel().states -= State.Setup.AvalonStart
                    channel().states -= State.Setup.KittensStart
                    gameType.startGame(message)
                }
            } else {
                if (State.Setup.AvalonStart in channel().states || State.Setup.KittensStart in channel().states) {
                    // todo link to the voting message
                    message.reply(" You are already voting to start the game here!", ping = true)
                    return@with
                }
                ...
                GlobalScope.launch {
                    ...
                    suspendUntil(500) {
                        ...
                        val voteScore = setup.startVote?.second ?: return@suspendUntil false
                        if (voteScore != setup.players.size) return@suspendUntil false
    ...
    fun voteExecute(gameType: GameType): suspend Bot.(MessageReactionUpdatePayload) -> Unit = { reaction ->
        ...
        val (voteMessage, i) = setup.startVote ?: null to 0

        if (reaction.user().isBot != true && message == voteMessage) {
            ...
            setup.startVote = message to (i + delta)
        }
    }
